...
function incrBinaryClassification( N, options ) {
	...
	if ( !isPositiveInteger( N ) ) {
		throw new TypeError( format( '0h14b,Ht', N ) );
	}
	opts = {
		'intercept': true,
		'lambda': 1.0e-4,
	...
	*/
	function accumulator( x, y ) {
		...
		if ( !isVectorLike( x ) ) {
			throw new TypeError( format( '0h14c,PZ', x ) );
		}
		if ( y !== -1 && y !== 1 ) {
			throw new TypeError( format( '0h14d,Pa', y ) );
		}
		if ( x.shape[ 0 ] !== model.nfeatures ) {
			throw new TypeError( format( '0h14e,Pb', model.nfeatures, x.shape[ 0 ] ) );
		}
		model.update( x, y );
		return model.coefficients;
	}
	...
	*/
	function predict( X, type ) {
		...
		var t;
		if ( !isndarrayLike( X ) ) {
			throw new TypeError( format( '0h14f,Pc', X ) );
		}
		sh = X.shape;
		if ( sh[ sh.length-1 ] !== N ) {
			throw new TypeError( format( '0h1Pd', N, sh[ sh.length-1 ] ) );
		}
		t = 'label';
		if ( arguments.length > 1 ) {
			if ( type === 'probability' ) {
				if ( opts.loss !== 'log' && opts.loss !== 'modifiedHuber' ) {
					throw new Error( format( '0h14h,Pe', [ 'log', 'modifiedHuber' ].join( '", "' ), opts.loss ) );
				}
			} else if ( type !== 'label' && type !== 'linear' ) {
				throw new TypeError( format( '0h14i,Pf', type ) );
			}
			t = type;
		}
