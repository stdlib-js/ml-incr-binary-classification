...
function validate( opts, options ) {
		...
		if ( options.learningRate.length > 1 ) {
			if ( name === 'constant' || name === 'invscaling' ) {
				...
				if ( !isPositiveNumber( opts.learningRate[ 1 ] ) ) {
					return new TypeError( format( '0h14n,Ph', 'learningRate', opts.learningRate[ 1 ] ) );
				}
			}
		}
		if ( options.learningRate.length > 2 ) {
			if ( name === 'invscaling' ) {
				...
				if ( !isNumber( opts.learningRate[ 2 ] ) ) {
					return new TypeError( format( '0h14o,Pi', 'learningRate', opts.learningRate[ 2 ] ) );
				}
			}
		}
	}
	if ( hasOwnProp( options, 'loss' ) ) {
		...
		if ( !contains( LOSS_FUNCTIONS, opts.loss ) ) {
			return new TypeError( format( '0h12X,3g,4S,GD,Gg,Jm', 'loss', LOSS_FUNCTIONS.join( '", "' ), opts.loss ) );
		}
	}
	return null;
}
